...
float4 frag(v2f i) : COLOR
{
    ...
    float4 col = float4(diffuseReflection, 1) * cookieAttenuation;
    UNITY_APPLY_FOG(i.fogCoord, col);

    if (_FogEnabled == 1.0) {
        float heightFog = 1.0 - exp(-(_FogHeight - i.worldPos.y) * _FogDensity);
        heightFog = saturate(heightFog);
        col.rgb = lerp(col.rgb, _FogColor.rgb, heightFog);
    }
